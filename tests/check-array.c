...
START_TEST(tb_array_new_check)
{
        ...
        tb_array_free(&array, NULL);
}
END_TEST

START_TEST(tb_array_free_check)
{
        TbArray *array = NULL;
...
static inline void tb_array_free_fun(void *p)
{
	free(p);
}

START_TEST(tb_array_add_check)
{
        TbArray *array = NULL;
        int data1 = 1;
        int data2 = 2;
        ...
        fail_if(tb_array_add(NULL, &data1),
                "Added data to NULL array");
        array = tb_array_new();
        fail_if(!array, "Failed to allocate new array");
        fail_if(tb_array_add(array, NULL),
                "Added NULL data to array");
        fail_if(!tb_array_add(array, &data1),
        ...
        array->len = 1;
        /* Test resize */
        fail_if(!tb_array_add(array, &data2),
...
START_TEST(tb_array_get_check)
{
        ...
        array = tb_array_new();
        fail_if(!array, "Failed to allocate new array");
        fail_if(tb_array_get(NULL, 0),
                "Got data from NULL array");
        fail_if(tb_array_get(array, 0),
                "Got data from empty array");
        fail_if(!tb_array_add(array, &data1),
                "Failed to add data1 to array");
        fail_if(*((int *)tb_array_get(array, 0)) != 1,
                "Failed to get correct value for element 0");
        fail_if(tb_array_get(array, 1),
...
START_TEST(tb_array_check)
{
        ...
        array = tb_array_new();
        fail_if(array == NULL, "Failed to allocate memory for TbArray");
        element = strdup("test");
        fail_if(!element, "Failed to allocate memory for array item");
        r = tb_array_add(NULL, element);
        fail_if(r, "Added element to NULL array");
        r = tb_array_add(array, NULL);
        fail_if(r, "Added NULL element to array");
        r = tb_array_add(array, element);
        fail_if(r  == false, "Failed to add element to TbArray");
        fail_if(array->len != 1,
                "Failed to get correct value for number of elements in array");

        f = tb_array_get(NULL, 0);
        fail_if(f, "Got value from NULL array");
        f = tb_array_get(array, (uint16_t)(array->len + 1));
        fail_if(f, "Got value from index bigger than maximum index");
        value = (char *)tb_array_get(array, 0);

        fail_if(value == NULL,
                "Failed to get value from TbArray");
...
int main(void)
{
	...
	s = suite_create("tb_array");
	tc = tcase_create("tb_array_functions");
	tcase_add_test(tc, tb_array_new_check);
	tcase_add_test(tc, tb_array_free_check);
	tcase_add_test(tc, tb_array_add_check);
	tcase_add_test(tc, tb_array_get_check);
	tcase_add_test(tc, tb_array_check);
