...
START_TEST(tb_hashmap_new_check)
{
        TbHashmap *map = NULL;

        map = tb_hashmap_new(NULL, NULL);
        fail_if(!map, "Failed to allocate new hashmap");
        fail_if(tb_hashmap_size(map) != 0, "Hashmap size is not 0 after new");
        tb_hashmap_free(map);
}
END_TEST

START_TEST(tb_hashmap_simple_check)
{
        TbHashmap *map = NULL;
        bool b;
        void *val = NULL;

        map = tb_hashmap_new(simple_hash, simple_compare);
        fail_if(!map, "Failed to allocate new hashmap");
        ...
        fail_if(tb_hashmap_size(map) >= 0, "Incorrect size returned for NULL hashmap");
}
END_TEST

START_TEST(tb_hashmap_string_check)
{
        TbHashmap *map = NULL;
        bool b;
        void *val = NULL;
...
START_TEST(tb_hashmap_iter_check)
{
        TbHashmap *map = NULL;
        TbHashmapIter iter;
        bool b;
        ...
        fail_if(tb_hashmap_size(map) != 5000, "Invalid hashmap size after 5000 elements");
        ...
        fail_if(tb_hashmap_size(map) != 3000, "Invalid hashmap size after removals");
...
START_TEST(tb_hashmap_alloc_check)
{
        TbHashmap *map = NULL;
        char *str = NULL;
        void *val = NULL;
...
static Suite *tb_hashmap_suite(void)
{
	Suite *s = NULL;
	TCase *tc = NULL;

	s = suite_create("tb_hashmap");
	tc = tcase_create("tb_hashmap_functions");
	tcase_add_test(tc, tb_hashmap_new_check);
	tcase_add_test(tc, tb_hashmap_simple_check);
	tcase_add_test(tc, tb_hashmap_string_check);
	tcase_add_test(tc, tb_hashmap_iter_check);
	tcase_add_test(tc, tb_hashmap_alloc_check);
